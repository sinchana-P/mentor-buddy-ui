...
import { UserPlus, GraduationCap, Edit, Trash2, AlertTriangle, LayoutGrid, Table2, Eye } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen w-full">
      ...
      <PageHeader
        ...
        stats={[
          { label: 'Total', value: buddies.length },
          { label: 'Active', value: buddies.filter(b => b.status === 'active').length },
        ]}
      />
      ...
      <FilterBar
        ...
        searchPlaceholder="Search buddies by name, email, or mentor..."
        filters={
          <>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="text-sm text-muted-foreground">
            Showing <span className="font-medium text-foreground">{buddies.length}</span> {buddies.length === 1 ? 'buddy' : 'buddies'}
          </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      ...
                      <th className="text-left py-3 px-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">Mentor</th>
                      <th className="text-left py-3 px-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">Days</th>
                      <th className="text-left py-3 px-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">Progress</th>
                      <th className="text-left py-3 px-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">Alerts</th>
                  ...
                  <tbody>
                    {(() => {
                      ...
                      <motion.tr
                        ...
                        className={`border-b border-border last:border-0 hover:bg-muted/50 transition-colors ${canViewDetails ? 'cursor-pointer' : ''}`}
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium text-foreground">{buddy.user?.name || buddy.name || 'Unknown'}</p>
                              <p className="text-xs text-muted-foreground">{buddy.user?.email || buddy.email}</p>
                            </div>
                          </div>
                        </td>
